...
DispatchedRequest.prototype.dispatchFile = function(filepath,file,replacements) {
	...
	var filename;
	logger.debug("Dispatching %s %s",filepath,file)
...
/*
		...
		if(err) {        
	  	...
	  	var contentType = mimeTypesByExtension[path.extname(filename)];
  		...
  		if ((contentType.indexOf("text/html")>-1) || (contentType.indexOf("application/json")) && (replacements != undefined)) {
    	...
    	*/
		...
		if(err) {        
	  	...
	  	logger.debug("Sending Content Type %s mode %s",contentType,mode)
	  	try {
	  		that.response.writeHead(200, headers);
	  		if (!that.response.finished) {
		  		that.response.write(file, mode);

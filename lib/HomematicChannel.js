...
const logger = require(path.join(__dirname, '/logger.js')).logger('HomematicChannel')
...
HomematicChannel.prototype.getChannelDescription = function(paramset) {
    ...
    result['INDEX'] = parseInt(this.index)
...
HomematicChannel.prototype.setParamsetValue = function(paramsetName, parameterName, value) {
    var that = this
    this.paramsets.forEach(function(pSet) {
        if (pSet.name === paramsetName) {
            logger.debug('Channel %s set Paramset %s Value %s to %s', that.index, paramsetName, parameterName, value)
...
HomematicChannel.prototype.publishUpdateEvent = function(parameterNames) {
    ...
    this.emit('logicevent_channel_value_change', {
        channel: this.adress,
        parameters: parameterNames
    })
}

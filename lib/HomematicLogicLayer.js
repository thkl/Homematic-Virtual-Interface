...
HomematicLogicalLayer.prototype.doReportUsageToCCU = function(ifname, address, value_id, count) {
    ...
    if ((this.ccuDPInUse[dpn] === undefined) ||  (this.ccuDPInUse[dpn].count !== count)) {
...
HomematicLogicalLayer.prototype.getValue = function(interf, adress, datapoint, callback) {
    let that = this
    let result = this.getCachedState(interf, adress, datapoint)
    if (result !== undefined) {
        callback(result)
    } else {
        that.callRPCMethod(interf, 'getValue', [adress, datapoint.toUpperCase()], function(error, value) {
            that.doCache(interf, adress, datapoint, value)
            callback(value)
        })
    }
}
    ...
    if (client) {
        try {
            client.methodCall(command, parameters, function(error, value) {
                if (callback) {
                    if (error) {
                        logger.error("RPC Command Error %s | on intf %s | for command %s | with %s", error, intf, command, parameters)
                    }
                    callback(error, value)
                }
            })

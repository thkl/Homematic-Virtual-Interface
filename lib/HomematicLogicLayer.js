...
var xmlrpc = require(path.join(__dirname,  "/homematic-xmlrpc"));
var request = require("request");
var logger = require(path.join(__dirname,  "/logger.js")).logger("HomematicLogicalLayer");
var HomematicChannel = require(path.join(__dirname,  "/HomematicChannel.js")).HomematicChannel;
var HomematicDevice = require(path.join(__dirname,  "/HomematicDevice.js")).HomematicDevice;
var regaRequest = require(path.join(__dirname,  "/HomematicReqaRequest.js"));
...
HomematicLogicalLayer.prototype.init = function() {
    ...
    'getDeviceDescription': function getDeviceDescription(err, params, callback) {
			...
			if (device) {
	    	    ...
	    	    logger.debug('(D)repl  > %s', JSON.stringify(re));
	    	    found = true;
	    	    callback(null,re)
	    	    return;
			} else {
				...
				if (channel) {
					...
					logger.debug('(C)repl  > %s', JSON.stringify(re));
					found = true;
					callback(null,re)
					return;
				} else {
...
HomematicLogicalLayer.prototype.addEventNotifier = function(callback) {
  ...
  if (this.rpcEventServer==undefined) {
    ...
    this.callRPCMethod('BidCos-RF','init', ["http://" + that.localip + ':' + port , 'hvl_BidCos-RF' ], function(error, value) {

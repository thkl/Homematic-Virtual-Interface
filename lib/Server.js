...
var logger = require(__dirname + "/logger.js")("Homematic Virtual Interface.Server");
...
Server.prototype.init = function() {
	...
	this.updateResult;
	this.myPathHandler=["/","/index/","/settings/","/install/","/remove/"];
	  ...
	  for (var i=0; i<configuredPlugins.length; i++) {
    	...
    	var plg = plugins[pluginType];
    	 ...
    	 if (plg!=undefined) {
		   ...
		   var pluginLogger = require(__dirname + "/logger.js")(pluginType + " - " + pluginName);
...
Server.prototype.checkUpdate = function() {
    ...
    try {	
	require('child_process').execSync('cd ' + appRoot + '/..;git remote update;git status').toString().trim();

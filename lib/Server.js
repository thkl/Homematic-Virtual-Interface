...
Server.prototype.handleConfigurationRequest = function(dispatched_request) {
	...
	logger.debug('Server HTTP Request %s parsed %s %s',requesturl,parsed.pathname,JSON.stringify(parsed.query))
	
	if (dispatched_request.request_paths[1] === 'index') {

	 if (parsed.query['installmode'] !== undefined) {
     ...
     if (parsed.query['checkupdate'] !== undefined) {
     ...
     if (parsed.query['doupdate'] !== undefined) {
	...
	if (parsed.query['cleanup'] !== undefined) {
	...
	if (parsed.query['showlog'] !== undefined) {
		var LoggerQuery = require(__dirname + '/logger.js').LoggerQuery
	...
	if (parsed.query['enabledebug'] !== undefined) {
	...
	if (parsed.query['disabledebug'] !== undefined) {
	...
	if (parsed.query['daemon'] !== undefined) {
		if (this.hazDaemon()) {
	...
	if (parsed.query['enabledaemon'] !== undefined) {
		this.enableDaemon()
	...
	if (parsed.query['disabledaemon'] !== undefined) {
		this.disableDaemon()    
	...
	if (parsed.query['logvirtualchannels'] !== undefined) {
		this.hm_layer.logVirtualChannels();
	}
	
	
	if (parsed.query['backup'] !== undefined) {
	...
	if (parsed.query['restart'] !== undefined) {
   	    setTimeout(function(){
	   	    that.restart()
	    }, 2000);
    	dispatched_request.dispatchFile(null,'restart.html',this.addDefaultIndexAttributes({'message':'Rebooting','link':'#'}))
	...
	if (parsed.query['getcorps'] !== undefined) {
		this.hm_layer.getcorpsedCCUDevices(function(list){
			  	...
			  	list.some(function(device){
					...
					devList = devList + dispatched_request.fillTemplate(template,{'device_address':device.address,'rega_id':device.regaid});
			  	})
				dispatched_request.dispatchFile(null,'corpsed_devices.html',that.addDefaultIndexAttributes({'list':devList}))
		})

		cfg_handled = true
	}	
	
	if (parsed.query['removecorps'] !== undefined) {
		logger.debug('remove unhandled devices')
       let regaid = parsed.query['regaid']
	   if (regaid !== undefined) {
		   logger.debug('trying to remove %s',regaid)
		   this.hm_layer.removeCorpses([{'regaid':regaid}],function(){
...
Server.prototype.buildpackagefile = function() {
  ...
  var pck = {"name": "hvl_install","scripts": {"start": "node node_modules/homematic-virtual-interface/lib/index.js || true","debug": "node node_modules/homematic-virtual-interface/lib/index.js -D || true"},"dependencies":deps}

	...
	if (parsed.pathname === '/settings/')
	{
    	...
    	if (parsed.query['ccu']!=undefined) {
	  	...
	  	if (parsed.query['addInterface']!=undefined) {
		  	var update = 'uhh something i never expect happened just now'
		  	...
		  	let scriptFile = path.join(__dirname,'ccu_rcscript.txt')
		  	...
		  	if (script) {
				...
				update = that.localization.localize('<a href=\'/settings/?rebootCCU\'>Please reboot your ccu</a>')
				dispatched_request.dispatchFile(null,'restart_ccu.html',that.addDefaultIndexAttributes({'message':update,'link':'#'}))
				cfg_handled = true
    		} else {
				logger.error('add script not found %s',scriptFile)
				dispatched_request.dispatchFile(null,'index.html',that.addDefaultIndexAttributes({'message':update,'link':'#'}))
				cfg_handled = true
				return
			}
		}
...
Server.prototype.doNPMUpdate = function(packageNames) {
	...
	try {
		...
		var exec = require('child_process').exec;
		var cmd = 'cd ' + destPath + ';' + that.npmCommand + ' ' + that.npmUpdate +' ' + ' --production --prefix "' + destPath + '"'
		require('child_process').execSync(cmd);
    }
    catch (err) {

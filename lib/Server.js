...
Server.prototype.doUpdate = function() {
    ...
    if (this.isNPM(npmName,appRoot)) {
		...
		logger.info("Update Core System at %s with %s",systemPath,cmd)
...
Server.prototype.doNPMUpdate = function(packageNames) {
	...
	try {
		...
		var exec = require('child_process').exec;
		var cmd = 'cd ' + destPath + ';' + that.npmCommand + ' ' + that.npmUpdate +' ' + ' --production --prefix "' + destPath + '"'		require('child_process').execSync(cmd);
    }
    catch (err) {

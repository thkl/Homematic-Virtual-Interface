		...
		if (parsed.query['plugin']!=undefined) {
			...
			try {
				this.installPlugin(plugin,function(error){
					that.plugins = that._loadPlugins()
		...
		this.fetchPluginList(function(error,lresult) {
			if (error==null) {
				...
				result = result + dispatched_request.fillTemplate(plugin_template,
			  														...
			  														'plugin.description':(pluginObject.description.de) || '',
			  														'plugin.installbutton':(pluginObject.installed)? 'disabled="disabled"':'',	
			  														'plugin.activatebutton':(pluginObject.active)? 'disabled="disabled"':'',	
  ...
  this.fetchPluginList(function (error,list){
	if (error==null) {
		list.some(function (pobj){
			logger.debug("%s vs %s",JSON.stringify(pobj),type)
			if (pobj.npm==type) {
...
Server.prototype.fetchPluginList = function(callback) {
   ...
   var url = 'https://raw.githubusercontent.com/thkl/Homematic-Virtual-Interface/master/plugins/plugins.json'
   var that = this
   var result = [];
   try {
   	...
   	request({url: url,json: true}, function (error, response, body) {
   	 if (!error && response.statusCode === 200) {
	   	 if ((body) && (body['plugins']))  {
		   	 result = body['plugins']
		   	 logger.debug("S %s",JSON.stringify(result))
...
Server.prototype.installNPM = function(npmName,destPath) {
	...
	var cmd = 'npm install ' + npmName + ' --production --prefix "' + destPath + '/"';

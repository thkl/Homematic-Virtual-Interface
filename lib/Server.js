...
Server.prototype.sendMessageToPlugin = function(pluginName, message, callback) {
    this.configuratedPlugins.forEach(plugin => {
        if (plugin.name === pluginName) {
            logger.debug('Sending message %s to %s', message.name, pluginName)
            plugin.platform.message(message, callback)
        }
    })
}

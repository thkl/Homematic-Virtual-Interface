...
Server.prototype.init = function() {
	...
	this.updateResult
	this.myPathHandler=['/','/index.html','/index/','/settings/','/install/','/device/','/plugins/']
			...
			switch (parsed.query['do']) {
				...
				case "version":
				   that.fetch_npmVersion(plgtp,function (centralVersion){
					   logger.debug("Get Version for %s result %s",plgtp,centralVersion)
					   dispatched_request.dispatchMessage('{"result":"'+ centralVersion +'"}')
				   })
				break;
			}
				cfg_handled = true
	...
	if (error==null) {
		list.some(function (pobj){
			if (pobj.npm==type) {
				...
				that.installNPM(pobj.npm,path.normalize(path.join(appRoot,'..','..','..')))
				that.reloadPlugins()
			}	
		})
...
Server.prototype.doUpdate = function() {
    ...
    var systemPath = path.normalize(path.join(appRoot,'..','..'))
    if (this.isNPM(npmName,path.normalize(path.join(appRoot,'..')))) {
   		var exec = require('child_process').exec;
		var cmd = 'cd ' + systemPath + ';' + this.npmCommand + ' install ' + npmName + ' --production --prefix "' + systemPath + '"';
...
Server.prototype.doNPMUpdate = function(packageNames) {
	...
	try {
		...
		var exec = require('child_process').exec;
		var cmd = 'cd ' + destPath + ';' + that.npmCommand + ' install ' + npmName + ' --production --prefix "' + destPath + '"';
		require('child_process').execSync(cmd);
    }
    catch (err) {
...
Server.prototype.isNPM = function(npmName,destPath) {
	...
	logger.debug("Checking NPM on %s",pjsonPath)
	try {
     var pjson = JSON.parse(fs.readFileSync(pjsonPath));
     logger.info("is NPM ? %s",pjson['_id'])
  ...
  httpUtil.httpCall("GET","https://registry.npmjs.org/" + pck + "/",[],function (data,error){
	try {
		if (data) {
			...
			if (registry) {
				if (callback) {callback(registry['dist-tags']['latest'])}
			} else {

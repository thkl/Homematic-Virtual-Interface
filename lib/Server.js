...
Server.prototype.handleConfigurationRequest = function(dispatched_request) {
    ...
    case "/index/?installmode":
        this.hm_layer.sendRPCMessage(undefined,"newDevices",this.hm_layer.getMyDevices(), function(error, value) {
			dispatched_request.dispatchFile(null,"action.html",this.addDefaultIndexAttributes({"message":"all devices published"}));
		});
        break;
	  ...
	  for (var i=0; i<configuredPlugins.length; i++) {
    	...
    	var plg = plugins[pluginType];
    	 ...
    	 if (plg!=undefined) {
		   ...
		   var instance = new plg.initializer(this,pluginName,pluginLogger);
		   instance.pluginPath = plg.pluginPath;
		   logger.info(instance.name +" initialized. Document Path is %s",instance.pluginPath);
		   this.configuratedPlugins.push(instance);
		   foundOnePlugin = true;
    	 }  else {

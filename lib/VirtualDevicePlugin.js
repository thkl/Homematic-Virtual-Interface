...
var fs = require('fs')
var logger = require(path.join(__dirname, 'logger.js')).logger('VirtualPlugin')
var Installer = require(path.join(__dirname, 'Installer.js'))
...
VirtualDevicePlugin.prototype.load = function (options) {
  ...
  // first Check if we have to install new dependencies
  var inst = new Installer(this.pluginPath, true)
...
VirtualDevicePlugin.loadPackageJSON = function (pluginPath) {
  ...
  try {
    ...
    // logger.debug("Plugin " + pluginPath + " contains an invalid package.json. Error: " + err);
  ...
  if (!pjson.name || pjson.name.indexOf('homematic-virtual-') !== 0) {
    // logger.debug("Plugin " + pluginPath + " does not have a package name that begins with 'homematic-virtual-'.");
  ...
  if (!pjson.keywords || pjson.keywords.indexOf('homematic-virtual-plugin') == -1) {
    //  logger.debug(" package.json does not contain the keyword 'homematic-virtual-plugin'");
  ...
  for (var index in VirtualDevicePlugin.paths) {
    ...
    for (var index2 in names) {
      ...
      if (!pluginsByName[type]) {
        ...
        var plgObject = new VirtualDevicePlugin(pluginPath)
        plgObject.version = pjson.version
        plugins.push(plgObject)
      } else {
        logger.warn("Warning: skipping plugin found at '" + pluginPath + "' since we already loaded the same plugin from '" + pluginsByName[type] + "'.")
      }
    }
  }

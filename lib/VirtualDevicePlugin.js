...
VirtualDevicePlugin.prototype.name = function() {
...
VirtualDevicePlugin.loadPackageJSON = function(pluginPath) {
  ...
  if (!pjson.name || pjson.name.indexOf('homematic-virtual-') != 0) {
	  logger.debugger("Plugin " + pluginPath + " does not have a package name that begins with 'homematic-virtual-'."sou);
  ...
  for (var index in VirtualDevicePlugin.paths) {
    ...
    for (var index2 in names) {
      ...
      try {
        ...
        if (!name || name.indexOf('homematic-virtual-') == 0) {

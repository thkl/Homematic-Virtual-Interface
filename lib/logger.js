...
function logConsoleString (prefix, level, message) {
  try {
    if ((prefix.match(DEBUG_FILTER)) || (message.match(DEBUG_FILTER)) || (level <= 2)) {
      if (chalk) {
        switch (level) {
          case 0 :
    ...
    console.log('Invalid Debug Filter %s', DEBUG_FILTER)
  }
}
...
function setFilter (filter) {
  console.log('Set LogFilter to %s', filter)
  DEBUG_FILTER = filter
}
...
LoggerQuery.prototype.clean = function (hours) {
  ...
  var that = this
  this.db.insert({ 'module': 'Logging System', 'time': new Date(), 'level': 2, 'msg': 'Will remove logentries older than ' + new Date(ts) })

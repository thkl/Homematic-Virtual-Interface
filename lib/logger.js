...
var logger = function (prefix) {
  ...
  if (db) {
	...
	clean : function(hours) {
		var ts = new Date().getTime() - (hours*3600000)
		db.insert({'module':"Logging System",'time':new Date(),'level':2,'msg':"Wil remove logentries older than " +  new Date(ts)})
		db.remove({time : { $lt: ts }}, { multi: true }, function (err, numRemoved) {
			db.insert({'module':"Logging System",'time':new Date(),'level':2,'msg':numRemoved + " Logentries removed"})
		});
	}

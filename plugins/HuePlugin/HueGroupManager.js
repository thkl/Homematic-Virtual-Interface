...
HueGroupManager.prototype.addGroup = function(group) {
    ...
    this.log.debug("Group properties %s", JSON.stringify(group.action))
    group["_id"] = id
    this.isColorGroup = (group.action.hue !== undefined)
    ...
    if (groups.length > 0) {
        groups.forEach(function(groupid) {
            ...
            if (group != undefined) {
                ...
                group["hm_device_name"] = "HUEGROUP" + that.instance + "0" + group["_id"]
                if (that.isColorGroup) {
                    that.hmDevices.push(new HueColorDevice(that, that.hueApi, group, "HUEGROUP" + that.instance + "0"))
                } else {
                    that.hmDevices.push(new HueDimmableDevice(that, that.hueApi, group, "HUEGROUP" + that.instance + "0"))
                }

...
var TradfriBlind = require("./TradfriBlind.js").TradfriBlind;
var TradfriBlindGroup = require("./TradfriBlindGroup.js").TradfriBlindGroup;
...
TradfriPlatform.prototype.reconnect = function() {
    ...
    function tradfri_deviceRemoved(instanceId) {
        // clean up
        if (that.trApiLightbulbs[instanceId])  {
            delete that.trApiLightbulbs[instanceId]
            that.log.info("Light removed: %s", instanceId)
        }
        if (that.trApiBlinds[instanceId]) {
            delete that.trApiBlinds[instanceId]
            that.log.info("Blind removed: %s", instanceId)
        }
    }
    ...
    function tradfri_groupUpdated(group) {
        ...
        if (!that.trApiGroups[group.instanceId]) {
            ...
            if (groupType === 2) {
                var tgroup = new TradfriBlindGroup(that, group.instanceId, group.deviceIDs) // (plugin, id)
                that.blindGroups[group.instanceId] = tgroup
            }
        } else {
...
TradfriPlatform.prototype.myDevices = function() {
    ...
    for (var index in this.trApiLightbulbs) {
        ...
        result.push({
            "id": 'Tradfri' + light.instanceId,
            "name": light.name,
            "udn": 'Tradfri' + light.instanceId,
            "type": "TRADFRI"
        })
    }
    ...
    for (var index in this.trApiGroups) {
        ...
        result.push({
            "id": 'Tradfri' + group.instanceId,
            "name": group.name,
            "udn": 'Tradfri' + group.instanceId,
            "type": "TRADFRI"
        })
    }
...
TradfriPlatform.prototype.updateBlindGroups = function() {
    let that = this
    Object.keys(this.blindGroups).forEach(groupid => {
        that.blindGroups[groupid].updateGroup()
    });
}
...
TradfriPlatform.prototype.showSettings = function(dispatched_request) {
    ...
    result.push({
        "control": "text",
        "name": "tradfri_securityid",
        "label": "Security ID",
        "value": this.securityID,
        "description": this.localization.localize("See backside of your bridge")
    });
    result.push({
        "control": "text",
        "name": "tradfri_ip",
...
TradfriPlatform.prototype.handleConfigurationRequest = function(dispatchedRequest) {
    ...
    for (var index in this.trApiBlinds) {
        deviceList = deviceList + dispatchedRequest.fillTemplate(devtemplate, {
            "device_hmdevice": String('Tradfri' + this.trApiBlinds[index].instanceId),
            "device_name": this.trApiBlinds[index].name,
            "device_type": this.trApiBlinds[index].deviceInfo.modelNumber,
            "device_spectrum": "n/a"
        });
    }

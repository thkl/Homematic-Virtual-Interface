...
var LightifyRGBDevice = function(plugin, api ,light,serialprefix) {
	...
	LightifyRGBDevice.prototype.setColor = function(newColor) {
		...
		if (co_channel != undefined) {
			...
			var g = result[2];
			var b = result[3];
			this.api.nodeColor(this.mac,r,g,b,255,this.transitiontime);
			co_channel.updateValue("RGBW",newColor);
		}
	}


	LightifyRGBDevice.prototype.setWhite = function(newTemp) {
	    var co_channel = this.hmDevice.getChannelWithTypeAndIndex("VIR-LG_RGBW-DIM-CH","1");
		if (co_channel != undefined) {
	...
	LightifyRGBDevice.prototype.setLevel = function(newLevel) {
	...
	LightifyRGBDevice.prototype.refreshDevice = function(device) {
	  var that = this;
	...
	module.exports = {
	  LightifyRGBDevice : LightifyRGBDevice
	}

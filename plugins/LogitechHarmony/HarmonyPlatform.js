...
HarmonyPlatform.prototype.init = function() {
    ...
    if (this.config.getValueForPluginWithDefault(this.name, "use_roku", false) == true) {
        this.rokuServer = new HarmonyRokuServer(this, 9093, "-ROKU")
...
HarmonyPlatform.prototype.showSettings = function(dispatched_request) {
    ...
    result.push({
        "control": "text",
        "name": "hub_ip",
        "label": this.localization.localize("Harmony Hub IP"),
        "value": hub_ip,
        "size": 20
    });
    ...
    result.push({
        "control": "text",
        "name": "localHostIP",
        "label": this.localization.localize("Local Host IP (if not the first Interface)"),
        "value": localHostIP,
        "size": 20
    });
...
HarmonyPlatform.prototype.saveHarmonyObjects = function(objectsToSave, callback) {
    this.config.savePersistentObjektToFile({
        'harmony_objects': objectsToSave
    }, 'harmony_objects', callback);
}
    ...
    this.flobjects.some(function(lightdevice) {
        ...
        if (type != undefined) {
            if ((editId != undefined) && (lightdevice["index"] == editId)) {
                ...
                fakeLights = fakeLights + dispatched_request.fillTemplate(lighttemplatefakeEdit, {
                    "lamp_name": lightdevice["name"],
                ...
                fakeLights = fakeLights + dispatched_request.fillTemplate(lighttemplatefake, {
                    "lamp_name": lightdevice["name"],
                    "lamp_index": lightdevice["index"],
                    "hm_device_type": type
                });
...
HarmonyPlatform.prototype.buildCCUObjectList = function(dispatched_request, editId) {
    ...
    this.flobjects.some(function(lightdevice) {
        ...
        if (type != undefined) {
            ...
            fakeLights = fakeLights + dispatched_request.fillTemplate(template, {
                "lamp_name": lightdevice["name"],
                "lamp_index": lightdevice["index"],
                "hm_device_type": type,
...
HarmonyPlatform.prototype.listHarmonyCommands = function(dispatched_request) {
    ...
    result.some(function(action) {
        list = list + dispatched_request.fillTemplate(atmp, {
            "action": action
        });
    });
    return list;
    ...
    this.hm_layer.loadCCUProgramms(function(jobj) {
        if (jobj) {
            jobj.programs.forEach(function(program) {
                result_list["P:" + program.id] = {
                    "device": program.name,
                    "address": "P:" + program.id,
                    "name": program.name
                };
            })
            callback(result_list)
        ...
        if (jobj) {
            jobj.variables.forEach(function(variable) {
                if ((variable.type == 2) && (variable.type == 2)) {
                    result_list["V:" + variable.id] = {
                        "device": variable.name,
                        "address": "V:" + variable.id,
                        "name": variable.name
                    };
                }
            })
...
HarmonyPlatform.prototype.handleConfigurationRequest = function(dispatchedRequest) {
    ...
    if (lightdevices != undefined) {
        lightdevices.forEach(function(lightdevice) {
            if (lightdevice.isReal == true) {
                realLights = realLights + dispatchedRequest.fillTemplate(lighttemplatereal, {
                    "lamp_name": lightdevice.name,
                    "lamp_index": lightdevice.index
                });
            }
        ...
        case "fake.new":
            {
                ...
                fakeLights = fakeLights + dispatchedRequest.fillTemplate(lighttemplatefake, {
                    "lamp_name": "New Device",
        ...
        case "ccu_object.new":
            {
                ...
                ccuObjects = ccuObjects + dispatchedRequest.fillTemplate(lighttemplatefake, {
                    "lamp_name": "New Device",
                    "lamp_index": (50 + this.flobjects.length)
                });
        ...
        case "send.command":
            {
                ...
                if (cmdName) {
                    this.harmonyClient.sendAction(cmdName)
                }
                var list = this.listHarmonyCommands(dispatchedRequest)
                dispatchedRequest.dispatchFile(this.plugin.pluginPath, 'list_actions.html', {
                    'actions': list
        ...
        case "device.list":
            {
                this.loadCCUDevices(function(result) {
                    dispatchedRequest.dispatchFile(that.plugin.pluginPath, "list.json", {
    ...
    this.harmonyClient.getActivities().forEach(function(activity) {
        activityList = activityList + dispatchedRequest.fillTemplate(activityTemplate, {
            "ac_name": activity.label,
            "ac_index": activity.chid,
            "ac_id": activity.id
        });
    });
    ...
    if (this.use_roku == true) {
        ...
        if (cmdMap) {
            Object.keys(cmdMap).forEach(function(m) {
                rokuElements = rokuElements + dispatchedRequest.fillTemplate(lighttemplatereal, {
                    "lamp_name": cmdMap[m],
                    "lamp_index": m
                });
            })
        }
        rokuList = dispatchedRequest.fillTemplate(rokutmp, {
            "rokuList": rokuElements
        });
    }


    dispatchedRequest.dispatchFile(this.plugin.pluginPath, "index.html", {
        "listRealLights": realLights,

...
MQTTPlatform.prototype.loadDevices = function () {
  ...
  if (odev) {
    try {
	  ...
	  this.log.error('load devices error %s', e.stack)
    }
  }
}
...
MQTTPlatform.prototype.initMqttConnection = function () {
  ...
  this.clientID = this.clientID + Math.random().toString(36).substring(2, 15)
  var host = this.configuration.getValueForPlugin(this.name, 'broker_host')
  ...
  if (host != undefined) {
    ...
    try {
      ...
      that.log.error('initMqttConnection Error %s', e.stack)
      return
    }
...
MQTTPlatform.prototype.publish = function (topic, value) {
  if (this.mqttClient !== undefined) {
	  this.log.debug('trying to publish %s to %s', value, topic)
	  this.mqttClient.publish(topic, value)
  } else {
	 this.log.error('mqtt client not here')
  }
}

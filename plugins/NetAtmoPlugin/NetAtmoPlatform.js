...
NetAtmoPlatform.prototype.connectApi = function(auth) {
    ...
    api.getStationsData(function(err, devices) {
        ...
        devices.forEach(function(device) {
            that.log.debug('Found Device %s', JSON.stringify(device))
            var hazCombo = false
...
NetAtmoPlatform.prototype.showSettings = function(dispatched_request) {
    ...
    result.push({
        'control': 'text',
        'name': 'client_id',
        'label': this.localization.localize('Client ID'),
        'value': client_id,
        'size': 30
    })
    result.push({
        'control': 'password',
        'name': 'client_secret',
        'label': this.localization.localize('Client Secret'),
        'value': client_secret,
        'size': 30
    })
    result.push({
        'control': 'text',
        'name': 'username',
        'label': this.localization.localize('Username'),
        'value': username,
        'size': 30
    })
    result.push({
        'control': 'password',
        'name': 'password',
        'label': this.localization.localize('Password'),
        'value': password,
        'size': 30
    })
    ...
    result.push({
        'control': 'text',
        'name': 'var_rain',
        'label': this.localization.localize('CCU Rain variable'),
        'value': ccu_rain,
        'size': 30
    })
    ...
    result.push({
        'control': 'text',
        'name': 'var_sunshine',
        'label': this.localization.localize('CCU Sunshine variable or datapoint'),
        'value': ccu_sunshine,
        'size': 30
    })
    result.push({
        'control': 'text',
        'name': 'var_sunshine_factor',
        'label': this.localization.localize('CCU Sunshine Factor'),
        'value': ccu_sunshine_factor,
        'size': 30
    })
    ...
    result.push({
        'control': 'text',
        'name': 'var_bright',
        'label': this.localization.localize('CCU brightness variable or datapoint'),
        'value': ccu_bright,
        'size': 30
    })
    result.push({
        'control': 'text',
        'name': 'var_bright_factor',
        'label': this.localization.localize('CCU brightness factor'),
        'value': ccu_bright_factor,
        'size': 30
    })
    ...
    result.push({
        'control': 'text',
        'name': 'var_co2_var',
        'label': this.localization.localize('CCU CO2 Variable'),
        'value': co2_var,
        'size': 30
    })
...
NetAtmoPlatform.prototype.handleConfigurationRequest = function(dispatched_request) {
    ...
    this.devices.forEach(function(device) {
        strDevice = strDevice + dispatched_request.fillTemplate(devicetemplate, {
            'device_name': device.name,
            'device_hmdevice': device.hm_device_name
        })
    })
